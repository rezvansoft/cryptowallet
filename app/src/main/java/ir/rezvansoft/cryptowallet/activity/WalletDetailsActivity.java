...
public class WalletDetailsActivity extends AppCompatActivity {
    ...
    private Thread deleteWalletThread=new Thread(new Runnable() {
        ...
        public void run() {
            ...
            if(deletedCount>0)handler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(getApplicationContext(),"کیف پول حذف شد",Toast.LENGTH_SHORT).show();
                }
            });
        }
    });
    ...
    private void deleteWallet()
    {
        ...
        builder.setMessage("حذف کیف پول '"+wallet.getDisplayName()+"' ?")
    ...
    private void updateUI(Wallet wallet) {
        ...
        if (wallet==null)
        {
            Toast.makeText(this,"ببخشید! مشکلی پیش آمده",Toast.LENGTH_SHORT).show();
            finish();
            return;
        }
